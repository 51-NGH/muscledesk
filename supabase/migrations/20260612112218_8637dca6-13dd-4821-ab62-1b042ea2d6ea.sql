...
BEGIN
  ...
  -- Per-tier breakdown: configured limit + gym count + member usage
  SELECT jsonb_agg(t ORDER BY t.plan) INTO _plan_limits FROM (
    SELECT
      pl.plan::text AS plan,
      pl.member_limit,
  ...
  -- Average bytes per member (table+indexes for members table / member count)
  IF _total_members > 0 THEN
    _avg_bytes_per_member := pg_total_relation_size('public.members')::numeric / _total_members;
  ELSE
